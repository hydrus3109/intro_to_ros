...
class ROVController(Node):
    ...
    def arm_response_callback(self, future):
        try:
            ...
            if response.success:
                self.get_logger().info('Arming successful' if future.result().success else 'Disarming successful')
            else:
...
def main(args=None):
    ...
    rov_controller.send_arm_command(True)  # Change to False to disarm
